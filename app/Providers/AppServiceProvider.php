...
class AppServiceProvider extends ServiceProvider
{
    ...
    private function rateLimiters(): void
    {
        RateLimiter::for('api', function (Request $request) {
            return [
                Limit::perMinute(30*2)->by($request->user()?->id ?: $request->ip()),
                Limit::perDay(1000*5)->by($request->user()?->id ?: $request->ip())
            ];
        });
        ...
        RateLimiter::for('check_verification_code', function (Request $request) {
            return Limit::perDay(30*2)->by($request->user()?->id ?: $request->ip());
        });
        ...
        RateLimiter::for('change_password', function (Request $request) {
            return Limit::perDay(12)->by($request->user()?->id ?: $request->ip());
        });
        RateLimiter::for('web', function (Request $request) {
            return Limit::perMinute(30)->by($request->user()?->id ?: $request->ip());
        });
        RateLimiter::for('refresh_token', function (Request $request) {
            return Limit::perDay(3*5000)->by($request->user()?->id ?: $request->ip());
        });
